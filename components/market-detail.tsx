...
export function MarketDetail({ market, onClose, onBuyYes, onBuyNo }: MarketDetailProps) {
  ...
  const load = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      setBets((data.bets ?? []).slice().reverse()) // most recent first
  ...
  // Chart data: add current point at the end
  const chartData = history.length > 0
    ? [
        ...history.map((h, i) => ({ t: i, y: h.yesPercent })),
        { t: history.length, y: yesPercent },
      ]
    : [{ t: 0, y: yesPercent }]
    ...
    <div className="fixed inset-0 z-40 flex flex-col bg-background animate-in slide-in-from-bottom-full duration-300">
      ...
      <div className="shrink-0 border-b border-border px-4 h-[57px] flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {isHot && (
            <span
              className="shrink-0 text-[9px] font-black text-orange-400 uppercase tracking-wider px-1.5 py-0.5 bg-orange-500/15 border border-orange-500/30 animate-pulse"
              style={{ borderRadius: "var(--radius-badge)" }}
            >
              🔥 HOT
            </span>
          ...
          {hasMomentum && !isHot && (
            <span
              className="shrink-0 text-[9px] font-bold text-accent uppercase tracking-wider px-1.5 py-0.5 bg-accent/10 border border-accent/20"
              style={{ borderRadius: "var(--radius-badge)" }}
            >
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-4 pt-4 pb-32 flex flex-col gap-5">
          ...
          <div className="flex items-stretch gap-3">
            <div
              className="flex-1 flex flex-col items-center justify-center py-5 bg-success/5 border border-success/20"
              style={{ borderRadius: "var(--radius-card)" }}
            >
              <span className="font-mono text-5xl font-black text-success tabular-nums leading-none">
                {yesPercent.toFixed(1)}%
              </span>
              <span className="mt-1.5 text-[10px] text-success/60 uppercase tracking-widest font-semibold">YES</span>
            </div>
            <div
              className="flex-1 flex flex-col items-center justify-center py-5 bg-danger/5 border border-danger/20"
              style={{ borderRadius: "var(--radius-card)" }}
            >
              <span className="font-mono text-5xl font-black text-danger tabular-nums leading-none">
                {noPercent.toFixed(1)}%
              </span>
          ...
          <div className="relative h-1.5 bg-muted overflow-hidden" style={{ borderRadius: "9999px" }}>
            <div
              className="absolute inset-y-0 left-0 bg-success transition-all duration-700"
              style={{ width: `${yesPercent}%` }}
            />
            <div
              className="absolute inset-y-0 right-0 bg-danger transition-all duration-700"
          ...
          <div>
            ...
            {chartData.length > 1 ? (
              <div
                className="bg-surface border border-border overflow-hidden px-1 pt-3 pb-1"
                style={{ borderRadius: "var(--radius-card)" }}
              >
              ...
              <div
                className="bg-surface border border-border px-4 py-6 text-center"
                style={{ borderRadius: "var(--radius-card)" }}
              >
          ...
          {market.userBet && (
            <div
              className="flex items-center justify-between px-4 py-3 bg-accent/5 border border-accent/30"
              style={{ borderRadius: "var(--radius-button)" }}
            >
              <div>
                <p className="text-[10px] text-accent uppercase tracking-widest font-semibold mb-0.5">Your Position</p>
                <p className="text-sm font-mono font-bold text-foreground">
                  {market.userBet.side.toUpperCase()} · {formatCredits(market.userBet.amount)} CR
                </p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5">Current odds</p>
                <p className="font-mono text-sm font-bold text-accent">
                  {market.userBet.side === "yes" ? yesPercent.toFixed(1) : noPercent.toFixed(1)}%
                </p>
              </div>
            </div>
          )}
      ...
      {!isResolved && !market.userBet && (
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] z-50 bg-background border-t border-border px-4 py-3 flex gap-3">
          <button
            onClick={onBuyYes}
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-success text-success-foreground font-bold text-sm uppercase tracking-wide transition-all hover:bg-success/90"
            style={{ borderRadius: "var(--radius-button)" }}
          >
          ...
          <button
            onClick={onBuyNo}
            className="flex-1 flex items-center justify-center gap-2 py-3 bg-danger text-danger-foreground font-bold text-sm uppercase tracking-wide transition-all hover:bg-danger/90"
            style={{ borderRadius: "var(--radius-button)" }}
          >
